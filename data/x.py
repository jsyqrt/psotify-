...
ALBUM_PATH_TEMPLATE=          'hm://album/v1/album-app/album/spotify:album:%s/desktop?catalogue=free&locale=it&username=%s'
ARTIST_PATH_TEMPLATE=          'hm://artist/v1/%s/desktop?format=json&catalogue=premium&locale=it&username=%s'
...
class MercuryManager( threading.Thread ):
  ...
  def _parse_response( self, payload ):
    return int.from_bytes(payload[2:10], byteorder='big'), payload[0:15], payload#header, parts
...
class spotify_artist_info:
  ...
  def get_artist_info_by_id(self, artist_id, artist_info_callback):
    self.manager.execute(REQUEST_TYPE.GET, 
                                   ARTIST_PATH_TEMPLATE % (artist_id, self.username),
                                   artist_info_callback)
  def reconnect(self):
...
def output_result(json_artist_result):
    ...
    playcount = set()
    for album in albums:
        for track in albums.get('discs')[0].get('tracks'):
            playcount.add((name, uri, album.get('name'), album.get('uri'), track.get('name'), track.get('uri'), track.get('popularity'), track.get('playcount')))
...
if __name__ == '__main__':
  sai = spotify_artist_info('jsyqrt', 'jsyqrt_xhwj')
  #artist_ids = ['6eUKZXaKkcviH0Ku9w2n3V']
  artist_ids = ["2m4rcnifN39Q2ix2vbpLlx", "3HXSUfI76zVZk71UMAeVfp", "2lbUjuMzHkT4qIECvk2f3L", "4SklOYXOJe2H6R1Vz2gc0F", "3GlCTpkvYItP16MDPC1Gnd", "1YrtUPrWcPfgdl9BaD9nhd", "7v7bP8NfcMYh4sDADEAy6Z", "7mShFo2eYgVBQFZBCilhtl", "4ntvojSPscU3PxselAEeY2", "6CVUpi9pFojARyeQLMPjTG", "3yfDRplDZh03Y0fEeCd6B0"]
  def pp(seq_id, x):
      x=x[-1].decode('utf8', errors='ignore')
      if x.find('hm://artist/') in range(15, 20):
        x=x[x.find('{"uri": "spotify:artist:'):]
        ARTIST_DATA[seq_id] = x
      else:
        if ord(x[14]) > 32:
          x=x[14:]
        else:
      ...
      if x.endswith('}}'):
          pp.finished = True
          result = ARTIST_DATA[seq_id]).lstrip('0')
          print(result)
      else:
          pp.finished = False

  for id in artist_ids:
    sai.get_artist_info_by_id(id, pp)
